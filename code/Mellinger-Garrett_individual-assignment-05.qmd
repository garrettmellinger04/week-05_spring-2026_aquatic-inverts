...
copepods <- ggplot(data = copepod_df,
  ...
  labs(x = "Site",
       ...
       title = "Copepods average abundance by site") +
  # changing theme
  theme_bw()
...
```{r}
#| label: wrangling salinity abundance data

# create new dataframe from aq_ins_clean
salinity_abundance_df <- aq_ins_clean |> 
  # keep columns needed for visualization
  select(taxon,
         ave_lit,
         med_sal) |> 
  # create new columns for plotting
  mutate(log_abundance = log(ave_lit + 1),
         # making sure each taxon in title case
...
```{r}
#| label: visualizing salinity abundance
#| fig-width: 12
#| fig-height: 10
